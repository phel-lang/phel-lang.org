...
A form Phel now says another way was used as source.

**Common cause:** `php/new`, `php/->`, `php/::` or `set-var` was written by hand. The compiler still emits them, but they are no longer accepted in source.
...
Raised while compiled code runs, not while it compiles: the program was well formed and the values it met were not.

### PHEL400 : Runtime not callable

A value that is not a function was called at runtime.

**Common cause:** The head of the list is a name or an expression the analyzer could not check, and it evaluated to a non-callable value.

**Fix:** Call a function, or read the value with an accessor such as `get`, `nth` or `first`.

### PHEL401 : Runtime arity error

A function was called with the wrong number of arguments at runtime.

**Common cause:** The callee is a value the analyzer has no arity for, such as a local closure, so the mismatch only surfaces when it runs.

**Fix:** Pass one argument per declared parameter.

### PHEL402 : Runtime type error

A value of the wrong type reached a PHP function or method.

**Common cause:** Most of these come from `php/` interop calls whose callee declares a type the argument does not satisfy.
...
An indexed read asked for a position the collection does not have.

**Common cause:** `nth` (and other indexed reads) past the end of a collection. `get` returns `nil` instead of throwing.

**Fix:** Check the index against `(count coll)`, or use `get` with a default.

### PHEL404 : Division by zero

A division or remainder had zero on the right.

**Common cause:** `/`, `%`, `rem` and `php/intdiv` all raise it when the divisor is zero.
