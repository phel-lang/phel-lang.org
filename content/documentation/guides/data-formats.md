...
Clojure-aligned runtimes, or with itself across process boundaries:
...
> Looking for JSON or CSV instead? The [Cookbook](/documentation/guides/cookbook)
> has ready-to-run recipes for reading a CSV file and parsing JSON via `phel.json`.
> Use EDN/Transit when you need Phel's richer value types (keywords, sets, symbols,
> tagged literals) to survive the round trip.

## `phel.edn`

EDN is the right choice for Phel-to-Phel config files, source-like data, and
human-edited input that benefits from comments and `#_` discard forms.
...
`phel.edn` delegates reading to Phel's own reader and writing to
`Printer::readable()`, so **every value Phel can print readably round-trips**:
nil, booleans, integers, floats, strings, characters, keywords, symbols, lists,
vectors, maps, sets, the `#_` discard form, `;` comments, and built-in tagged
literals (`#uuid`, `#inst`, `#regex`).

The lexer accepts EDN's full tag grammar: namespaced tags (`#my.app/Person`,
`#myapp/double`, `#my.app.module`) lex as a single tag, and unqualified tags
(`#uuid`, `#inst`) work unchanged.
...
Transit is the right choice for talking to Clojure, Java, or JS over HTTP, and
for round-tripping JSON-shaped data that carries rich types.
...
This first iteration implements **JSON-Verbose** only (no key caching): maps with
string keys serialise to ordinary JSON objects; any other map serialises as a
`~#cmap`.
...
Plain Phel strings whose first character is `~`, `^`, or `` ` `` are escaped on
write with a leading `~`, so the reader does not mistake them for a Transit tag.
...
- **Transit+MessagePack**.
- **Write-side extension hooks**: there is no symmetric `:handlers` map on
  `write-string` yet. Encode custom types yourself before handing them to
  `write-string`.
...
**See also:** [Cookbook](/documentation/guides/cookbook) for JSON and CSV recipes,
and the [full edn / transit / json API reference](/documentation/reference/api/edn/).
