...
`coerce` works on keyword keys. Turn string-keyed input into keywords first (for example with `keywordize-keys` from `phel.walk`). The [Schema Validation page](/documentation/libraries/schema/) covers the full API, including `instrument!` for checking function arguments.
