...
```phel
(->> (reduce (fn [acc _] (conj acc (+ (peek acc) (get acc (- (count acc) 2))))) [0 1] (range 2 10)))
...
```phel
(let [n 17] (and (> n 1) (every? |(not= 0 (% n $)) (range 2 (php/intval (+ 1 (php/sqrt n)))))))
# => true
```
...
```phel
(let [base 2 exp 10] (reduce (fn [acc _] (* acc base)) 1 (range 0 exp)))
# => 1024
```
...
```phel
(->> (seq "functional programming") (filter |(contains? (set "a" "e" "i" "o" "u") $)) count)
# => 6
```
...
```phel
(->> (str/split "hello world of phel" "/ /") (map str/capitalize) (str/join " "))
# => "Hello World Of Phel"
```
...
```phel
(->> (seq "Hello") (map (fn [c] (let [o (php/ord c)] (cond (and (>= o 65) (<= o 90)) (php/chr (+ 65 (% (+ (- o 65) 13) 26))) (and (>= o 97) (<= o 122)) (php/chr (+ 97 (% (+ (- o 97) 13) 26))) :else c)))) (apply str))
# => "Uryyb"
```
...
```phel
(reduce (fn [acc item] (assoc acc (get item :id) item)) {} [{:id 1 :name "Alice"} {:id 2 :name "Bob"}])
...
```phel
(->> [{:role "admin"} {:role "user"} {:role "admin"} {:role "user"} {:role "user"}] (group-by :role) (map-indexed (fn [_ [k v]] [k (count v)])))
# => (["admin" 2] ["user" 3])
```
...
```phel
(->> [{:name "A" :score 42} {:name "B" :score 99} {:name "C" :score 71}] (sort-by :score) reverse (take 2))
...
```phel
(merge {:host "localhost" :port 3306 :db "test"} {:port 5432 :db "prod"})
...
```phel
(->> [{:cat "a" :v 10} {:cat "b" :v 20} {:cat "a" :v 30}] (group-by :cat) (reduce (fn [acc [k items]] (assoc acc k (reduce + 0 (map :v items)))) {}))
# => {"a" 40 "b" 20}
```
...
```phel
(->> (frequencies [:alice :bob :alice :carol :bob :alice]) (sort-by second) reverse)
...
```phel
(map (fn [n] (cond (= 0 (% n 15)) "FizzBuzz" (= 0 (% n 3)) "Fizz" (= 0 (% n 5)) "Buzz" :else n)) (range 1 21))
...
```phel
(->> (seq "Attack at dawn") (map (fn [c] (let [o (php/ord c)] (cond (and (>= o 65) (<= o 90)) (php/chr (+ 65 (% (+ (- o 65) 3) 26))) (and (>= o 97) (<= o 122)) (php/chr (+ 97 (% (+ (- o 97) 3) 26))) :else c)))) (apply str))
# => "Dwwdfn dw gdzq"
```
...
```phel
(->> "Hello World, This is Phel!" (str/lower-case) (str/replace " " "-") (str/replace "/[^a-z0-9-]/" ""))
...
```phel
(loop [n 12 acc []] (if (= n 1) (conj acc 1) (recur (if (even? n) (/ n 2) (+ 1 (* 3 n))) (conj acc n))))
...
```phel
(->> (concat (range 1 6 2) (range 3 0 -2)) (map |(str/join "" [(str/repeat " " (/ (- 5 $) 2)) (str/repeat "*" $)])) (str/join "\n"))
# => "  *\n ***\n*****\n ***\n  *"
```
